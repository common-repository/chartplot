...
export class SelectedLegendItem extends AbstractSelectedComponent{
    ...
    getModel(model: EChartSettings){
        ...
        return model.components.getLegendAtIndex(index);
    }

    activate(){
        this.editorSettings.options.ribbon.selectedTab = RibbonOptions.COMPONENT_RIBBON_INDEX;
        this.editorSettings.options.components.selected = this.settings.components.components.values.indexOf(this.getModel(this.settings));
        this.editorSettings.options.chart.editMode = "component";
    }

}

export const indexLegend = createStandardIndexer(() => new SelectedLegendItem());
