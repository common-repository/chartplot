...
export class YAxisComponent extends AbstractSelectedComponent{
    ...
    getModel(model: EChartSettings): GridAxis{
        ...
        return model.coordinates.getAxisAtIndex(index, "y");
    }
    ...
    activate(){
        ...
        this.editorSettings.options.coordinates.selected = this.settings.coordinates.coordinates.values.indexOf(this.getModel(this.settings).grid) || 0;
    ...
    calculateBoundingRect(component){
        ...
        return {
            x: Math.round(xs), y: Math.round(ys), width: Math.round(xe - xs), height: Math.round(ye - ys)
        };
    }

}

YAxisComponent.prototype.type = "yAxis";

export const indexYAxis = createStandardIndexer(() => new YAxisComponent());
